...
class SlidingWindowRateLimiter:
    ...
    def _cleanup_window(self, user_id: str, current_time: float) -> None:
        if user_id in self.storage:
            while (
                ...
                and self.storage[user_id][0] <= current_time - self.window_size
            ):
    ...
    def can_send_message(self, user_id: str) -> bool:
        ...
        if (
            user_id not in self.storage
            or len(self.storage[user_id]) < self.max_requests
        ):
            return True
    ...
    def time_until_next_allowed(self, user_id: str) -> float:
        ...
        if user_id not in self.storage or len(self.storage[user_id]) == 0:
            return 0.0
        return self.storage[user_id][0] + self.window_size - current_time
